...
suite('Performance Tests - Large Datasets', () => {
    ...
    setup(async function() {
        ...
        testDbPath = path.join(beadsDir, 'test-performance.db');
    ...
    suite('Large Dataset Loading', () => {
        test('Load board with 500 issues - full load', async function() {
            ...
            const board = await adapter.getBoard(1000); // Load up to 1000
        ...
        test('Load board with 1000 issues - incremental loading', async function() {
            ...
            // Load with incremental limit (100 per column)
            const startLoad = Date.now();
            const board = await adapter.getBoard(400); // 100 per column max
            const loadTime = Date.now() - startLoad;

            console.log(`Loaded ${board.cards.length} issues with limit 400 in ${loadTime}ms`);
    ...
    suite('Incremental Loading Performance', () => {
        test('Load More performance - single column page', async function() {
            ...
            const ids1 = page1.cards.map(c => c.id);
            const ids2 = page2.cards.map(c => c.id);
        ...
        test('Parallel column loading performance', async function() {
            ...
            console.log(`Loaded: ready=${ready.cards.length}, in_progress=${inProgress.cards.length}, blocked=${blocked.cards.length}, closed=${closed.cards.length}`);
    ...
    suite('Filtering Performance', () => {
        ...
        test('Search query performance on large dataset', async function() {
            ...
            const board = await adapter.getBoard(1000);
    ...
    suite('Debouncing and Batching', () => {
        ...
        test('Rapid mutations should coalesce saves', async function() {
            ...
            const final = board.cards.find(c => c.id === issue.id);
    ...
    suite('Worst-Case Scenarios', () => {
        test('All issues in single column (closed)', async function() {
            ...
            assert.ok(closedData.cards.length === 100, 'Should load exactly 100 issues');
            assert.ok(closedData.totalCount >= 500, 'Total count should reflect full column');
        });
        ...
        test('All issues with many labels and dependencies', async function() {
            ...
            const startLoad = Date.now();
            const board = await adapter.getBoard(200);
            const loadTime = Date.now() - startLoad;
        ...
        test('Very long issue titles and descriptions', async function() {
            ...
            const startLoad = Date.now();
            const board = await adapter.getBoard(100);
            const loadTime = Date.now() - startLoad;
