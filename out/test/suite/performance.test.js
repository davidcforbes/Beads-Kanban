...
suite('Performance Tests - Large Datasets', () => {
    ...
    async function createManyIssues(count, statusDistribution) {
    ...
    suite('Filtering Performance', () => {
        ...
        test('Search query performance on large dataset', async function () {
            ...
            try {
                ...
                const filtered = (board.cards || []).filter(card => card.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        test('Multiple filter combination performance', async function () {
            ...
            try {
                ...
                const filtered = (board.cards || []).filter(card => (card.status === 'open' || card.status === 'in_progress') &&
                    card.priority <= 2 &&
    ...
    suite('Debouncing and Batching', () => {
        ...
        test('Batched issue creation performance', async function () {
            ...
            try {
                ...
                const promises = [];
                for (let i = 0; i < 100; i++) {
                    promises.push(adapter.createIssue({
                        title: `Batch Issue ${i}`,
