...
**Bold** and *italic* text.`;
    ...
    suite('Read Operations', () => {
        ...
        test('Get column count returns accurate counts', async function () {
            ...
            try {
                ...
                await adapter.createIssue({ title: 'In Progress', description: '', status: 'in_progress' });
    ...
    suite('Update Operations', () => {
        ...
        test('Update issue type', async function () {
            this.timeout(10000);
            try {
                const result = await adapter.createIssue({ title: 'Test', description: '', issue_type: 'task' });
                await adapter.updateIssue(result.id, { issue_type: 'bug' });
                const board = await adapter.getBoard();
                const updated = (board.cards || []).find(c => c.id === result.id);
